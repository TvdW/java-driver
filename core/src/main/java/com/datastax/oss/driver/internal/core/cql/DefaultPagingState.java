...
public class DefaultPagingState implements PagingState {
  ...
  public static DefaultPagingState fromBytes(byte[] bytes) {
    ...
    if (rawPagingStateLength + hashLength != buffer.remaining()
        && rawPagingStateLength + hashLength + 2 != buffer.remaining()) {
  ...
  // Hashes a statement's query string and parameters. In retrospect we probably didn't need to
  // include the paging state in the hash, but keeping it to have exactly the same output as driver
  // 3.
